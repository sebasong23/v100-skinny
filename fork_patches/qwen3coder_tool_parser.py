...
#
# Patched: this parser emits Qwen `<tool_call>...<function=...>` XML, never
# OpenAI JSON function definitions, but it previously declared
# supports_required_and_named=True. That flag makes the serving layer
# short-circuit `tool_choice: "required"`/named requests into JSON-based
# reconstruction (engine/serving.py `_parse_tool_calls_from_content`). With
# that, the XML it emits is consumed as JSON: it either drops the call entirely (`tool_calls: []` with
# `finish_reason: tool_calls`) or passes the raw XML through as `arguments` —
# both surface on the client as empty args / INVALID_ARGS. The XML extractor
# works for every tool-choice mode (verified over the wire), so the flag is
# forced to False to route required/named through the XML parser.
import json
import uuid
...
class Qwen3CoderToolParser(ToolParser):
    # Routed through the XML extractor for every tool-choice mode. The base
    # class accepts True (JSON reconstruction of required/named calls), which
    # this parser cannot serve — see header note.
